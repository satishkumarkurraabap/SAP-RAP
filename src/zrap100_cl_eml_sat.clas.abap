CLASS zrap100_cl_eml_sat DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zrap100_cl_eml_sat IMPLEMENTATION.
  ...
  METHOD read_travel.
    ...
    DATA travels TYPE TABLE FOR READ IMPORT ZR_RAP_ATRAV_SAT000.
...
*        FIELDS ( AgencyID CustomerID BeginDate Description )
    ...
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed read = { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ENDIF.
  ...
  METHOD update_travel.
    ...
    MODIFY ENTITIES OF ZR_RAP_ATRAV_SAT000
      ENTITY ZrRapAtravSat000
...
*    COMMIT ENTITIES
*      RESPONSE OF ZR_RAP_ATRAV_SAT000
...
*      REPORTED DATA(reported_commit).
    ...
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed update = { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
...
*        FROM VALUE #( ( %cid_ref = 'create_travel' %is_draft = instance_state ) )
    ...
    console_output->write( mapped-ZrRapAtravSat000 ).
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed create: { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ELSEIF failed_commit IS NOT INITIAL.
      console_output->write( |- Cause for failed commit: { failed_commit-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ENDIF.
    console_output->write( |--------------------------------------------- | ).
  ENDMETHOD.
 ...
 METHOD delete_travel.
    ...
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed delete: { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ELSEIF failed_commit IS NOT INITIAL.
      console_output->write( |- Cause for failed commit: { failed_commit-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ENDIF.
    console_output->write( |--------------------------------------------- | ).
  ...
  METHOD activate_travel_draft.
    ...
    console_output->write( |- TravelID = { travel_id } | ).
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed activate: { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ELSEIF failed_commit IS NOT INITIAL.
      console_output->write( |- Cause for failed commit: { failed_commit-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ENDIF.
    console_output->write( |--------------------------------------------- | ).
  ...
  METHOD discard_travel_draft.
    ...
    console_output->write( |- TravelID = { travel_id } | ).
    IF failed IS NOT INITIAL.
      console_output->write( |- Cause for failed discard: { failed-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ELSEIF failed_commit IS NOT INITIAL.
      console_output->write( |- Cause for failed commit: { failed_commit-ZrRapAtravSat000[ 1 ]-%fail-cause } | ).
    ENDIF.
    console_output->write( |--------------------------------------------- | ).
